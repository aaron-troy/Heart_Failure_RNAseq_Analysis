...
- Expression signatures associated with one or more conditions of interest. These can be simply normalized counts or the results of DEGs analsysis, as in the case here. We will use the DESeq results with 3 paired contrasts between control, HFpEF, and HFrEF
...
#### Retrieve a high confidence prior knowledge human TF network (PKN) from doRothea
...
#### Use the human protein atlas to remove any TFs not present in the heart
```{r removeNonHeartTFs}
...
```
#### Now we can run msVIPER for TF enrichment on the DEG data  
...
#### Compute the cosine similarity to get composite measure of regulons unique to HFpEF
...
#### Inspect the results with a few plots of the top TFs for each contrast
...
## Switch gears and now perform the same analysis, but sample by sample
